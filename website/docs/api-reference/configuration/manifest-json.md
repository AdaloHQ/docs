...
| `formValue`         |      | Instead of the normal value, this prop will pass a `{ value, onChange, initial }` object that can be used like a react [Controlled Component](). `value` will initially be given the value passed to the prop and can have its value exported throughout the app, `onChange` is the event handler function that can be used to update `value`, and `initial` is the initial value passed to the prop that will not change when `onChange` is called. |                                                                                    
